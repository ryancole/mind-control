...
/// and how far it reaches, in world units. Reference facts handed to the
...
/// table so the distance to a target is a fact and not a guess.
/// </summary>
public static class AbilityKits
{
    public sealed record Note(string Slot, string Kind, double? Range);

    private static readonly Dictionary<string, Note[]> Kits = new()
    {
        ["Ezreal"] =
        [
            new Note("Q", "Mystic Shot: a skillshot poke", 1150),
            new Note("W", "Essence Flux: a skillshot", 1200),
            new Note("E", "Arcane Shift: a blink, not something to throw at an enemy", null),
